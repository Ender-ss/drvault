...
import { AdSpyFeed } from '../components/spy/AdSpyFeed'

export default function MartinsSpy() {
  const [activeGroup, setActiveGroup] = useState<string>('ad_spy')
  ...
  const openInspector = (title: string, url: string, niche?: string, category?: string, thumbUrl?: string) => {
    setInspectData({
      isOpen: true,
      title,
      url,
      niche: niche || currentNiche?.title || 'Geral',
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight text-white flex items-center gap-2">
                MARTINS SPY <span className="text-xs font-semibold px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 uppercase">BigSpy & Pipads Live</span>
              </h1>
      ...
      {/* RENDER AD SPY FEED IF AD_SPY GROUP IS ACTIVE */}
      {activeGroup === 'ad_spy' ? (
        <AdSpyFeed />
      ) : (
        <>
              ...
              {activeSubtab === 'tt_kw' && (
                <div className="space-y-6">
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1.5 flex-shrink-0">
                                ...
                                <button
                                  onClick={() => openInspector(kw, url, currentNiche.title, 'broll')}
                                  className="p-1.5 rounded bg-pink-500/20 hover:bg-pink-500/40 text-pink-400 transition-colors flex items-center gap-1 text-xs"
                                  title="Assistir no Player Interno"
                                >
                  ...
                  {currentNiche.tiktok_links && currentNiche.tiktok_links.length > 0 && (
                    <div className="pt-4 border-t border-white/5">
                      <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-3">Links Diretos de Vídeos TikTok</h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                        {filteredLinks(currentNiche.tiktok_links).map((link, idx) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              <button
                                onClick={() => openInspector(link.label, link.url, currentNiche.title, 'broll')}
                                className="p-1.5 rounded bg-pink-500/20 hover:bg-pink-500/40 text-pink-400 transition-colors"
                                title="Reproduzir vídeo no DRVault"
                              >
                                <Play className="w-3.5 h-3.5 fill-current" />
                              </button>
                              <a
                    ...
                    {currentNiche.tiktok_profiles?.map((p, idx) => (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between text-[10px] text-gray-500 mt-3 pt-2 border-t border-white/5">
                          <span>{p.followers || 'Perfil monitorado'}</span>
                          <div className="flex items-center gap-1.5">
                            <button
                              onClick={() => openInspector(p.name, p.url, currentNiche.title, 'avatar')}
                              className="p-1 rounded bg-pink-500/20 text-pink-400 hover:bg-pink-500/40 transition-colors"
                              title="Inspecionar no DRVault"
                            >
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between text-[10px] text-gray-500 mt-3 pt-2 border-t border-white/5">
                          ...
                          <div className="flex items-center gap-1.5">
                            <button
                              onClick={() => openInspector(p.name, p.url, currentNiche.title, 'avatar')}
                              className="p-1 rounded bg-purple-500/20 text-purple-400 hover:bg-purple-500/40 transition-colors"
                              title="Inspecionar no DRVault"
                            >
                              <Play className="w-3 h-3 fill-current" />
                            </button>
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="p-1 rounded text-purple-400 hover:text-purple-300"
                            >
                              <ExternalLink className="w-3.5 h-3.5" />
                            </a>
                          </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {currentNiche.reels?.map((r, idx) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          <button
                            onClick={() => openInspector(r.label, r.url, currentNiche.title, 'broll')}
                            className="p-1.5 rounded bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/40 transition-colors"
                            title="Assistir no Player do DRVault"
                          >
                            <Play className="w-3.5 h-3.5 fill-current" />
                          </button>
                          <a
      ...
      {inspectData && (
        <MediaInspectorModal
          isOpen={inspectData.isOpen}
          onClose={() => setInspectData(null)}
          title={inspectData.title}
          url={inspectData.url}
          niche={inspectData.niche}
          category={inspectData.category}
          thumbUrl={inspectData.thumbUrl}
        />
      )}
    </div>
  )
